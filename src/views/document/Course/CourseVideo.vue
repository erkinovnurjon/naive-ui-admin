  ...
  <div class="min-h-screen bg-gray-50 p-4">
    <n-grid cols="1 m:3" responsive="screen" :x-gap="24" :y-gap="24">
        ...
        <n-card :bordered="false" class="main-card">
          <n-space vertical size="large">
            ...
            <div class="video-section">
              <!-- Processing Message -->
              <div v-if="processingMessage" class="processing-wrapper">
                <n-result status="warning" size="large">
                  <template #icon>
                  ...
                  <template #footer>
                    <n-alert type="info" :show-icon="false">
                      <n-text>{{ processingMessage }}</n-text>
                    </n-alert>
                  </template>
                </n-result>
              </div>
              ...
              <div
                ...
                class="video-container aspect-video w-full rounded-lg overflow-hidden bg-black"
              >
                <Player
                  :show-is-short="false"
                  :url="currentVideoSrc"
              ...
              <!-- No Video State -->
              <div v-else class="no-video-wrapper p-12 text-center">
                <n-empty description="Video qo'shilmagan" size="large">
                  <template #icon>
                    <n-icon size="80" color="#d4d4d8">
                      <svg viewBox="0 0 24 24">
                        <path
                          fill="currentColor"
                          d="M21 3H3c-1.11 0-2 .89-2 2v14c0 1.11.89 2 2 2h18c1.11 0 2-.89 2-2V5c0-1.11-.89-2-2-2zM21 19H3V5h18v14zM13.5 13.5L10 16.5l-2.5-2L4 18h16l-4.5-6z"
                        />
                        <path
                          fill="currentColor"
                          d="M2 2l20 20M22 2L2 22"
                          stroke="currentColor"
                          stroke-width="2"
                        />
                      </svg>
                  ...
                  <template #extra>
                    <n-text depth="3">
                      Hozirda ushbu dars uchun video mavjud emas
                    </n-text>
                  </template>
                </n-empty>
              </div>
            ...
            <div class="course-info-section">
              <n-space vertical size="medium">
                ...
                <div class="description-section">
                  <n-card size="small" embedded v-if="videoData.details">
                    <template #header>
                      <n-text strong>Kurs haqida</n-text>
                    </template>
                  ...
                  <n-alert v-else type="default" :show-icon="false">
                    <n-text depth="3">Kurs tavsifi kiritilmagan</n-text>
                  </n-alert>
                </div>

                <!-- Statistics -->
                <n-card title="Kurs statistikasi" size="small" embedded>
                  <n-grid
                    cols="2 s:4"
                    responsive="screen"
                    :x-gap="16"
                  ...
                  >
                    ...
                    <n-grid-item>
                      <div
                        class="stat-item flex items-center gap-3 p-3 bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg text-white"
                      >
                        <div
                          class="stat-icon bg-white/20 px-1.5 pt-1.5 rounded-full"
                        >
                    ...
                    <n-grid-item>
                      <div
                        class="stat-item flex items-center gap-3 p-3 bg-gradient-to-r from-green-500 to-green-600 rounded-lg text-white"
                      >
                        <div
                          class="stat-icon bg-white/20 px-1.5 pt-1.5 rounded-full"
                        >
                    ...
                    <n-grid-item>
                      <div
                        class="stat-item flex items-center gap-3 p-3 bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg text-white"
                      >
                        <div
                          class="stat-icon bg-white/20 px-1.5 pt-1.5 rounded-full"
                        >
                        ...
                        <div class="stat-content">
                          <div class="text-xs opacity-90">Jami darslar</div>
                          <div class="font-semibold">
                            {{ totalLessonsCount }}
                          </div>
                        </div>
                    ...
                    <n-grid-item>
                      <div
                        class="stat-item flex items-center gap-3 p-3 bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg text-white"
                      >
                        <div
                          class="stat-icon bg-white/20 pr-2 pt-1 rounded-full"
                        >
                          <n-icon size="20" color="white">
                            <svg viewBox="0 0 24 24">
                              <path
      ...
      <n-grid-item span="1 m:1">
        <n-card :bordered="false" class="h-full bg-white shadow-sm">
          <!-- Course Topics -->
                  ...
                  <template #header>
                    <div class="flex items-center gap-3 w-full py-1">
                      <n-tag
                        ...
                        size="small"
                        round
                        class="topic-number flex-shrink-0"
                      >
                        {{ topicIndex + 1 }}
                      </n-tag>
                      ...
                      <template #suffix>
                        <div class="flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {{
                              formatDuration(
                                lesson.videoFiles?.[0]?.duration || 0
                              )
                            }}
                          </span>
                          <n-icon
          ...
          <div v-else class="no-topics-wrapper p-8 text-center">
            ...
            >
              ...
              <template #extra>
                <n-text depth="3">
                  Administrator tomonidan tez orada darslar qo'shiladi
                </n-text>
              </template>
            </n-empty>
          </div>
...
const selectLesson = (lesson: any) => {
  ...
  if (currentVideo.value?.id === lesson.id) {
    // Same lesson clicked - toggle play/pause
  ...
  // Auto expand topic
  const parentTopic = videoData.value?.courseTopics?.find((t: any) =>
    t.videoLessons?.some((l: any) => l.id === lesson.id)
  );

  if (parentTopic && !expandedTopics.value.includes(parentTopic.id)) {
    expandedTopics.value.push(parentTopic.id);
  }
...
watch(currentVideo, (newVideo) => {
  if (newVideo) {
    document.title = `${newVideo.title} - ${videoData.value.title}`;
  }
});
</script>
