      ...
      <n-card class="shadow-sm">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-2">
            ...
            <h6 class="text-base font-semibold m-0">Fan sillabusi</h6>
          </div>
          <n-button
        ...
        <div v-if="practice.fileId" class="mt-3">
          <div class="bg-gray-50 p-3 rounded-lg">
            <div class="flex justify-between items-center">
              <div class="flex items-center gap-3">
                <div
                  class="w-10 h-10 rounded flex items-center justify-center text-white"
...
import { NCard, NButton, NModal, NIcon, NSpin, useMessage } from "naive-ui";
import {
  Add as AddIcon,
  Close as CloseIcon,
  Eye as EyeIcon,
...
const emitUpdate = () => {
  emit("update", localItems.value[0]);
};
...
const handleMaterialUpload = async (event: Event, index: number) => {
  ...
  formData.append("EntityName", "courseSyllabus");
...
const getFileIcon = (file: ILessonPractice) => {
  ...
  // Barcha fayl turlari uchun DocumentIcon ishlatamiz
  return DocumentIcon;
};
...
const getFileUrl = (fileId: string) => {
  return `${core_cdn.value}/Media/Get?entityName=courseSyllabus&fileId=${fileId}`;
};
...
onMounted(() => {
  if (!core_cdn.value) {
    console.warn("core_cdn.value mavjud emas! useAppUrl ishlamayapti.");
  }
});
</script>
